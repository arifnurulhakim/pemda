...
class Persampahan21Model extends Model
{

  protected $table = 'persampahan21';
  protected $primaryKey = 'id_persampahan21';
  ...
  function countAll()
  {
    return $this->db->table('persampahan21')->countAll();
  }

  public function getPersampahan21($slug = false)
  {
    if ($slug === false) {
      // $this->where(['slug' => $slug])->first();
      $this->join('perangkat_daerah', 'persampahan21.id_pd = perangkat_daerah.id_pd');
      $this->join('satuan', 'persampahan21.id_satuan = satuan.id_satuan');
      $this->join('kode_rekening', 'persampahan21.kode_rekening = kode_rekening.id_kode_rekening');
