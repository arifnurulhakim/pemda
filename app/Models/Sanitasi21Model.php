...
class Sanitasi21Model extends Model
{

  protected $table = 'sanitasi21';
  protected $primaryKey = 'id_sanitasi21';
  ...
  function countAll()
  {
    return $this->db->table('sanitasi21')->countAll();
  }

  public function getSanitasi21($slug = false)
  {
    if ($slug === false) {
      // $this->where(['slug' => $slug])->first();
      $this->join('perangkat_daerah', 'sanitasi21.id_pd = perangkat_daerah.id_pd');
      $this->join('satuan', 'sanitasi21.id_satuan = satuan.id_satuan');
      $this->join('kode_rekening', 'sanitasi21.kode_rekening = kode_rekening.id_kode_rekening');
